...
class Riwayat_pengeluaran extends CI_Controller {
    ...
    public function get_ajax(){
        ...
        $this->model_admin->column_search = array('pengeluaran.KdPengeluaran', 'pengeluaran.TglProses', 'pengeluaran.usrid');
        ...
        //Mulai
        $list = $this->model_admin->get_datatables_data_riwayat_pengeluaran("ms_pengeluaran", $where);
        $data = array();
        $no = @$_POST['start'];
        foreach ($list as $item){
            ...
            $row = array();
            $row[] = $no;
            $row[] = $item->KdPengeluaran;
            $row[] = date("D, Y-m-d H:i:s", $item->TglProses);
            $row[] = $item->usrid;
            $row[] = '<a href="'.base_url("adm/riwayat_pengeluaran/detail_pengeluaran/".$item->KdPengeluaran).'">
        ...
        $output = array(
            "draw"  => @$_POST['draw'],
            "recordsTotal"  => $this->model_admin->count_all_data_riwayat_pengeluaran($where),
            "recordsFiltered"   => $this->model_admin->count_filtered_data_riwayat_pengeluaran($where),
            "data" => $data
        );
    ...
    public function index(){
        ...
        $this->load->view('Admin/riwayat_pengeluaran');
        $this->load->view('Admin/Template_admin/footer');
    }

    public function detail_pengeluaran($Id = 0){
        if($Id === 0){
            $this->session->set_flashdata('pesan','<div class="alert alert-warning alert-dismissible" role="alert" style="color:#000">
                                                Harap Memilih terlebih dahulu data yang ingin dilihat!
                                                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                            </div>
            ');
            redirect('adm/riwayat_pengeluaran');
        }
        ...
        if(!$this->model_admin->cek_ada_tidak_sama($where, 'ms_pengeluaran')){
            $this->session->set_flashdata('pesan','<div class="alert alert-warning alert-dismissible" role="alert" style="color:#000">
        ...
        //Get Detail Data
        $data['detail_pengeluaran']       = $this->model_admin->get_data_pengeluaran_by_kd_pengeluaran($Id);
        $data['pengeluaran']              = $this->model_admin->get_data_from_uuid($where, "ms_pengeluaran")->row();
