...
class Riwayat_pengembalian extends CI_Controller {
    function keluar(){
    ...
    public function get_ajax(){
        ...
        //Mulai
        $list = $this->model_admin->get_datatables_data_riwayat_pengembalian("ms_pengembalian", $where);
        $data = array();
        $no = @$_POST['start'];
        ...
        $output = array(
            "draw"  => @$_POST['draw'],
            "recordsTotal"  => $this->model_admin->count_all_data_riwayat_pengembalian($where),
            "recordsFiltered"   => $this->model_admin->count_filtered_data_riwayat_pengembalian($where),
            "data" => $data
        );
    ...
    public function index(){
        ...
        $this->load->view('Admin/riwayat_pengembalian');
    ...
    public function cetak_pengembalian_range(){
        $tanggal_awal   = strtotime($this->input->post("tgl_input_real") . " 00:00:00");
        $tanggal_akhir  = strtotime($this->input->post("tgl_input_real_akhir") . " 00:00:00");

        $data['rekap'] = $this->model_admin->get_rekap_pengembalian($tanggal_awal, $tanggal_akhir);
        $data['rekap_per_kadar'] = $this->model_admin->get_rekap_pengembalian_per_kadar($tanggal_awal, $tanggal_akhir); 
        $data['tanggal_mulai']  = $this->input->post("tgl_input_real");
        $data['tanggal_akhir'] = $this->input->post("tgl_input_real_akhir");

        $this->load->view("Admin/print/cetak_pengembalian_per_hari_range", $data);
    }
    ...
    public function detail_pengembalian($Id = 0){
        if($Id === 0){
            ...
            ');
            redirect('adm/riwayat_pengembalian');
        }
        ...
        if(!$this->model_admin->cek_ada_tidak_sama($where, 'ms_pengembalian')){
            $this->session->set_flashdata('pesan','<div class="alert alert-warning alert-dismissible" role="alert" style="color:#000">
        ...
        //Get Detail Data
        $data['detail_pengembalian']        = $this->model_admin->get_data_pengembalian_by_kd_pengembalian($Id);
        $data['pengembalian']               = $this->model_admin->get_data_from_uuid($where, "ms_pengembalian")->row();

        $this->load->view("Admin/Template_admin/header");
        $this->load->view("Admin/Template_admin/sidebar");
        $this->load->view("Admin/lihat_data/riwayat_pengembalian", $data);
        $this->load->view('Admin/Template_admin/footer');
    }
}
